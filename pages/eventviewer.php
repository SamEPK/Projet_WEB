...
try {
    $sqlQuery = 'SELECT DISTINCT pseudo, desc, date_consultation, idClient FROM historique_client
                INNER JOIN evenement ON historique_client.idEvenement = evenement.id
                WHERE historique_client.idClient = :idClient
                GROUP BY historique_client.idEvenement, historique_client.date_consultation, pseudo, desc
...
foreach ($users as $user) {
    if (isset($user['idClient']) == $_SESSION['id']) {
        echo
        '</tr>' .
            '<tr>' . '<td>' . $user['pseudo'] . '</td>' . '<td>' 
            . $user['desc'] . '</td>' . '<td>'
